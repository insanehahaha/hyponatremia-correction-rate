...
add_table(main, pd.DataFrame(rows), "Table 2. Sodium correction rate and 30-day mortality by time origin", widths=[1.5, 1.4, 1.75, 1.65],
          footnote="Multivariable logistic regression adjusted for age (restricted cubic spline, 4 knots), sex, index serum sodium, prior sodium status, Charlson comorbidity index, eGFR, albumin, glucose, potassium, diuretic use before presentation and calendar year (17 degrees of freedom). "
          "Missing covariates were handled by multiple imputation. Standardized risks and risk differences were obtained by model-based standardization with patient-level bootstrap confidence intervals. Joint tests: " + joint + ". "
...
add_table(main, pd.DataFrame(rows)[COLS3], "Table 3. Age-specific adjusted risks of 30-day mortality and neurological deterioration by correction rate",
          ...
          f"Mortality model: the 17-degree-of-freedom core model plus linear age × correction-rate interaction terms (joint test for interaction: {test_p(summary.loc['landmark (primary)', 'd1_age_x_rate'])}). Neurological model: correction rate, age (restricted cubic spline), index serum sodium and intensive care unit admission within 24 h, plus linear age × correction-rate interaction terms (joint Wald test with robust covariance, p = {p_fg_int:.2f}); competing events were death and discharge without the event. "
          "In the ≥85-year group the neurological risk set comprises 99 patients with 8 events and the mortality analysis 110 patients with 22 deaths. Differences between age groups are assessed by the joint interaction test rather than by comparing individual groups. Confidence intervals in the oldest group do not exclude clinically important differences. CI, confidence interval.",
          section=section_row)
...
add_table(main, pd.DataFrame(rows)[C4], "Table 4. Sodium correction rate and secondary outcomes", widths=[1.9, 0.85, 1.75, 1.8],
          footnote="Reference category 4–8 mmol/L/24 h. Observed: Aalen–Johansen 14-day cumulative incidence (neurological deterioration), Kaplan–Meier estimate (mortality), median (interquartile range) days (DAOH-30) and proportion (readmission); for DAOH-30, n / events counts patients with DAOH-30 = 0. Neurological model: correction rate, age (restricted cubic spline), index serum sodium and intensive care unit admission within 24 h; competing events were death and discharge without the event; robust confidence intervals for sHR, bootstrap intervals for standardized estimates (cause-specific hazard ratios in Supplementary Table S3). "
          "Other models were adjusted for the core covariates listed in Table 2; missing covariates were handled by multiple imputation. Mortality at 90 days and 1 year: Cox regression with delayed entry at 24 h (proportional hazards assessment and restricted mean survival time in Supplementary Table S3). "
          "DAOH-30: whole days (0–30; death within 30 days scored 0); partial proportional-odds model with the proportional-odds constraint relaxed for the age terms; OR >1 indicates more days alive and out of hospital. "
...
add_table(supp, t1, "Supplementary Table S1. Characteristics of the study population by correction-rate category",
          ...
          "Baseline characteristics refer to the time of the index sodium measurement; glucose-corrected sodium: measured sodium + 1.6 mmol/L per 100 mg/dL of glucose above 100 mg/dL; eGFR and prior sodium status as defined in Table 1; medications before presentation are prescriptions within 90 days or active medication reports within 1 year. Care and treatment refer to the index care episode; hypertonic saline is any 3% sodium chloride order within 48 h of the index measurement. eGFR, estimated glomerular filtration rate; SNRI, serotonin–norepinephrine reuptake inhibitor; SSRI, selective serotonin reuptake inhibitor.",
          section=section_row)
...
add_table(supp, pd.DataFrame(rows)[C6], "Supplementary Table S3. Sensitivity, subgroup and additional analyses",
          "A: primary logistic model structure with the core covariates (Table 2); missing covariates handled by multiple imputation except in the complete-case analysis. In the episode-level analysis standard errors are clustered by patient. For the 48-h correction rate the exposure is derived from the measurement at 36–60 h, so the risk set is patients alive at 60 h. "
